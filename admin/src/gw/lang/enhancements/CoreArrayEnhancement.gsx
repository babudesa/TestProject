...
enhancement CoreArrayEnhancement<T> : T[] {
   ...
   * toList().
   * 
   * TODO cgross - expose this in Diamond when an ImmutableList interface
   * has been introduced
   */
  private function fastList() : java.util.List<T> {
  ...
  function sum<N extends java.lang.Number>( mapper(elt:T):N ) : N {
    return this.fastList().sum( mapper )
  }
   ...
   */
  function sortBy( value( elt : T ):Comparable ) : T[]
  {
    var comparator = new BlockSortByComparator( value, true )
    Arrays.sort( this as Object[], comparator )
    return this
  }
   ...
   */
  function sortByDescending( value( elt : T ):Comparable ) : T[]
  {
    var comparator = new BlockSortByComparator( value, false )
    Arrays.sort( this as Object[], comparator )
    return this
  }
