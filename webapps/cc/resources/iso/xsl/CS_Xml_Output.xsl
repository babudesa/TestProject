		...
		<HTML>
			...
			<BODY>
				...
				<TABLE border="0" cellpadding="0" cellspacing="0" bgcolor="white" width="680">
					...
					<br/>
					<div align="center">
						<font color="gray" face="arial" size="1">ISO Stylesheet Version: 5.2 Release Date: 02-04-2010
					</font>
					</div>
				</TABLE>
			</BODY>
	...
	<xsl:template name="Build_Property_Info">
		...
		<xsl:choose>
			<xsl:when test="count($PropLossNode/Watercraft) &gt; 0 or count($PropLossNode/ItemInfo) &gt; 0">
				...
				<xsl:variable name="Adjuster" select="$ClaimRoot/AdjusterParty/AdjusterPartyInfo[@AssignmentRef = $WaterCraftId or @AssignmentRef = $MobileId]"/>
				...
				<xsl:call-template name="Display_Water_Mobile_Property">
					...
					<xsl:with-param name="InvestigationNode" select="$ClaimRoot/InvestigationInfo[RecoveryInfo][@ItemRef = $WaterCraftId or @ItemRef = $MobileId]"/>
					<xsl:with-param name="ClaimsPayNode" select="$ClaimRoot/ClaimsPayment[@idref = $WaterCraftId or @idref = $MobileId][not(PaymentTypeCd)][ClaimsPaymentCovInfo/CoverageCd = $CoverageCode]"/>
