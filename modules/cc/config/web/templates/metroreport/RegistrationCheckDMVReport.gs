...
<mrb:MetroRequest xmlns:mrb="http://www.metroreporting.com">
  ...
  <mrb:Order>
		<mrb:RequestType>R</mrb:RequestType>
    ...
    <% } else { %>
      ...
      <mrb:DrLicenseNumber><%=claim.Insured.Person.LicenseNumber%></mrb:DrLicenseNumber>
      <mrb:LicenseState><%=claim.Insured.Person.LicenseState%></mrb:LicenseState>
    <% } %>
    ...
    <mrb:Driver1>
      <mrb:Address1><%=translator.formatDriverAddress(metroReport.VehicleIncident.driver.PrimaryAddress)%></mrb:Address1>
      <mrb:City><%=metroReport.VehicleIncident.driver.PrimaryAddress.City%></mrb:City>
      <mrb:State><%=metroReport.VehicleIncident.driver.PrimaryAddress.State%></mrb:State>
      <mrb:Zip><%=translator.formatNullToEmptyString(metroReport.VehicleIncident.driver.PrimaryAddress.PostalCode)%></mrb:Zip>
    ...
    <mrb:Vehicle1>
      <mrb:VehYear><%=metroReport.VehicleIncident.Vehicle.Year%></mrb:VehYear>
      <mrb:TagNumber><%=metroReport.VehicleIncident.Vehicle.LicensePlate%></mrb:TagNumber>
      <mrb:VehMake><%=metroReport.VehicleIncident.Vehicle.Make%></mrb:VehMake>
      <mrb:VehModel><%=metroReport.VehicleIncident.Vehicle.Model%></mrb:VehModel>
      <mrb:TagState><%=metroReport.VehicleIncident.Vehicle.State%></mrb:TagState>
      <mrb:VIN><%=metroReport.VehicleIncident.Vehicle.Vin%></mrb:VIN>
    </mrb:Vehicle1>
    <mrb:OrderingCompany>
