...
<mrb:MetroRequest xmlns:mrb="http://www.metroreporting.com">
  ...
  <mrb:Order>
		<mrb:RequestType>A</mrb:RequestType>
    ...
    <% } %>
      ...
      <mrb:Phone><%=translator.formatPhoneNumber(claim.Insured.PrimaryPhoneValue)%></mrb:Phone>
		...
		<mrb:Driver1>
    <% if ( metroReport.VehicleIncident.driver != null ) { %>
      <mrb:Last><%=metroReport.VehicleIncident.driver.LastName%></mrb:Last>
			<mrb:First><%=metroReport.VehicleIncident.driver.FirstName%></mrb:First>
			<mrb:Middle><%=translator.formatNullToEmptyString(metroReport.VehicleIncident.driver.MiddleName)%></mrb:Middle>
                        <mrb:DrLicenseNumber><%=metroReport.VehicleIncident.driver.LicenseNumber%></mrb:DrLicenseNumber>
                        <mrb:LicenseState><%=metroReport.VehicleIncident.driver.LicenseState%></mrb:LicenseState>
    <% } else { %>
        <mrb:Last>Parked</mrb:Last>
			  <mrb:First>Parked</mrb:First>
    <% } %>
    </mrb:Driver1>
    ...
    <% if (metroReport.VehicleIncident.Vehicle != null) { %>
    <mrb:Vehicle1>
      <mrb:VehYear><%=translator.formatNullToEmptyString(metroReport.VehicleIncident.Vehicle.Year)%></mrb:VehYear>
      <mrb:TagNumber><%=translator.formatNullToEmptyString(metroReport.VehicleIncident.Vehicle.LicensePlate)%></mrb:TagNumber>
      <mrb:VehMake><%=translator.formatNullToEmptyString(metroReport.VehicleIncident.Vehicle.Make)%></mrb:VehMake>
      <mrb:VehModel><%=translator.formatNullToEmptyString(metroReport.VehicleIncident.Vehicle.Model)%></mrb:VehModel>
      <mrb:TagState><%=translator.formatNullToEmptyString(metroReport.VehicleIncident.Vehicle.State)%></mrb:TagState>
      <mrb:VIN><%=translator.formatNullToEmptyString(metroReport.VehicleIncident.Vehicle.Vin)%></mrb:VIN>
    </mrb:Vehicle1>
    <% } %>
    <mrb:OrderingCompany>
