...
<mrb:MetroRequest xmlns:mrb="http://www.metroreporting.com">
  ...
  <mrb:Order>
		<mrb:RequestType>M</mrb:RequestType>
    ...
    <mrb:Insured>
    <% if (claim.Insured typeis Company) { %>
      <mrb:CompanyName><%=claim.Insured.Company.Name%></mrb:CompanyName>
      <mrb:DOB><%=metroReport.VehicleIncident.driver.DateOfBirth%></mrb:DOB>
    <% } else { %>
      ...
      <mrb:DOB><%=translator.formatCustDate(claim.Insured.Person.DateOfBirth)%></mrb:DOB>
    <% } %>
    ...
    <mrb:Driver1>
      <mrb:Last><%=translator.formatNullToEmptyString(metroReport.VehicleIncident.driver.LastName)%></mrb:Last>
			<mrb:First><%=translator.formatNullToEmptyString(metroReport.VehicleIncident.driver.FirstName)%></mrb:First>
			<mrb:Middle><%=translator.formatNullToEmptyString(metroReport.VehicleIncident.driver.MiddleName)%></mrb:Middle>
      <mrb:DOB><%=translator.formatCustDate(metroReport.VehicleIncident.driver.DateOfBirth)%></mrb:DOB>
    </mrb:Driver1>
    <mrb:Driver2>
      <mrb:Last><%=translator.formatNullToEmptyString(metroReport.ThirdPartyVehicle.driver.LastName)%></mrb:Last>
			<mrb:First><%=translator.formatNullToEmptyString(metroReport.ThirdPartyVehicle.driver.FirstName)%></mrb:First>
			<mrb:Middle><%=translator.formatNullToEmptyString(metroReport.ThirdPartyVehicle.driver.MiddleName)%></mrb:Middle>
      <mrb:Address1><%=translator.formatDriverAddress(metroReport.ThirdPartyVehicle.driver.PrimaryAddress)%></mrb:Address1>
      <mrb:City><%=translator.formatNullToEmptyString(metroReport.ThirdPartyVehicle.driver.PrimaryAddress.City)%></mrb:City>
      <mrb:Zip><%=translator.formatNullToEmptyString(metroReport.ThirdPartyVehicle.driver.PrimaryAddress.PostalCode)%></mrb:Zip>
      <mrb:DOB><%=translator.formatCustDate(metroReport.ThirdPartyVehicle.driver.DateOfBirth)%></mrb:DOB>
    </mrb:Driver2>
