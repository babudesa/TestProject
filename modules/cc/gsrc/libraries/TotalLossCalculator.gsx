  ...
  function TotalLossCalculatorPreUpdate() {
    ...
    if (this.Claim.LossType == LossType.TC_AUTO){
      ...
      if (newCalcValue > 25 ) {
        ...
        for(onlyFlaggingHistory in this.Claim.History.where( \ h -> h.Type == "flagged" )) {
          if(onlyFlaggingHistory.Description != null and onlyFlaggingHistory.Description.contains( displaykey.Web.TotalLossCalculatorPopup.TotalLossEarlyTow(this.DisplayName) )) {
        ...
        if(!wasFlaggedAsTotalLoss) {
          this.Claim.setFlagExt( displaykey.Web.TotalLossCalculatorPopup.TotalLossEarlyTow(this.DisplayName))        
        }
      ...
      // 25 or under
        ...
        this.Claim.removeFlagExt( displaykey.Web.TotalLossCalculatorPopup.TotalLossEarlyTow(this.DisplayName) )
      }
    }
  }
