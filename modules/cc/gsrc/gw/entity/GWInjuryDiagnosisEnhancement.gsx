...
enhancement GWInjuryDiagnosisEnhancement : entity.InjuryDiagnosis {
  ...
  function diagnosisCodeWarningString() : String {
    var str = displaykey.Rules.Validation.Claim.DiagnosisCode(this.ICDCode.Code)
    
    if (this.ICDCode.AvailabilityDate != null) {
        str = str + displaykey.Rules.Validation.Claim.DiagnosisCodeNotAvailable(this.ICDCode.AvailabilityDate.format("long"))
    }
    ...
    if (this.ICDCode.ExpiryDate != null) {
        str = str + displaykey.Rules.Validation.Claim.DiagnosisCodeExpires(this.ICDCode.ExpiryDate.format("long"))
    }
    return str
  }
}
