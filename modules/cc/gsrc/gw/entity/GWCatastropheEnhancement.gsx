...
enhancement GWCatastropheEnhancement : entity.Catastrophe {
  ...
  private function findClaims() : gw.api.database.Query<entity.Claim> {  
    ...
    q.subselect("ID", CompareNotIn, Activity, "Claim")
      .and( \ and1 -> {
        and1.compare("ActivityPattern", Equals , catastropherReviewActivityPattern)
        and1.or( \ or1 -> {
          or1.compareIn("Status", {ActivityStatus.TC_SKIPPED, ActivityStatus.TC_COMPLETE})
        })
       })
