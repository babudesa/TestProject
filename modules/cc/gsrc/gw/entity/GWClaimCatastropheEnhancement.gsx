...
enhancement GWClaimCatastropheEnhancement : entity.Claim {
  ...
  function getMatchingCatastropheMessage() : String {
    ...
    matches.reverse().eachWithIndex( \ c, i -> {     
      if (i == 0) {
        result = displaykey.Warning.Catastrophe.MatchingItem.Last( c.DisplayName )
      } else {
        result = displaykey.Warning.Catastrophe.MatchingItem( c.DisplayName, result )
      }   
    } )
    
    return displaykey.Warning.Catastrophe.ReviewMatches( result )
  }
  ...
  function getRelevantZoneTypes(countryVar : Country) : List<ZoneType> {
    ...
    return allTypeKeys.where( \ z -> (z.Categories.hasMatch( \ c -> c.Code == countryVar.Code) or z.Code == "country")).toList()
  }
}
