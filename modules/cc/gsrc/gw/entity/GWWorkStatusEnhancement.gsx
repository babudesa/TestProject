  ...
  function validateNonConflicting(data:EmploymentData) {
    for (ws in data.WorkStatusChanges) {
      if (this != ws and this.overlaps(ws)) {
        throw new DisplayableException(displaykey.Web.Claim.NewWorkStatusPopup.Conflict(this.StatusDate.format("short"), 
          this.StatusEndDate!=null ? this.StatusEndDate.format("short") : displaykey.Term.Date.Present))
      }
    }
  }
}
