 ...
 */
@ReadOnly
abstract class ClaimHealthCalculatorBatchBase extends BatchProcessBase {
  
  var _chunkSize : int
    
  /** Allows a configurable chunk size */
  ...
  /**
   *  Implement this method to return the Query to find the claims to be processed
   */
  abstract function findClaims() : IQueryBeanResult<Claim> 
  
  /**
   * Implement this method to perform the work on the claim to update the metrics
   */
  abstract function processClaim(claim : Claim)
  ...
  private function findNumberOfClaimsToProcess() : int {
    ...
    if (maxClaimResults < claimsWithNoMetricsCount) {
      PLLoggerCategories.SERVER_BATCHPROCESS.info(displaykey.Web.InternalTools.BatchProcess.ClaimHealthCalculator.ExceedsNumClaimResults(maxClaimResults, maxClaimResults, claimsWithNoMetricsCount))
  ...
  private function processClaimInNewBundle(claim : Claim) {
    try {
      gw.transaction.Transaction.runWithNewBundle( \ bundle -> {       
        processClaim(bundle.add(claim))
      })
    } catch (e : Exception) {
      PLLoggerCategories.SERVER_BATCHPROCESS.warn(displaykey.Web.InternalTools.BatchProcess.ClaimHealthCalculator.UpdateFailed(claim), e)
    }
  }
}
