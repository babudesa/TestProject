...
class AcordPolicyMapper implements IPolicyMapper
{
  ...
  override function populate(claimPolicy:Policy, req:ClaimsNotificationAddRq_Type) {
    ...
    if(policyElem.ContractTerm.EffectiveDt_elem!=null)
      claimPolicy.EffectiveDate = policyElem.ContractTerm.EffectiveDt_elem.toDate()
    if(policyElem.ContractTerm.ExpirationDt_elem!=null)
      claimPolicy.ExpirationDate = policyElem.ContractTerm.ExpirationDt_elem.toDate()
  }
  
    
  //returns the default currency for the claim
  private function getCurrency(req:ClaimsNotificationAddRq_Type):Currency {
