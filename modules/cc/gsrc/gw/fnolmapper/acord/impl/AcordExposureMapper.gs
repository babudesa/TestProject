...
class AcordExposureMapper implements IExposureMapper
{
  ...
  override function getExposure(claim:Claim, autoLossInfo:AutoLossInfo_Type, incident:VehicleIncident) : Exposure {
    var exposure:Exposure
    var coverageType = getCoverageType(autoLossInfo.Coverages)
    ...
    if(coverageType!=null) {
      ...
      if(incident.Collision) {
        //Collision
        exposure = newExposure(typekey.CoverageType.TC_COLL, CoverageSubtype.TC_COLL_VD, claim)
      } else if(containsPropertyOwner(autoLossInfo)) {
        //Property damage
        var coverageSubType = autoLossInfo.PropertyDamagedVehInd ?
          CoverageSubtype.TC_APD_VD:CoverageSubtype.TC_APD_PD
        exposure = newExposure(typekey.CoverageType.TC_APD, coverageSubType, claim)
      } else {
        //Comprehensive
        exposure = newExposure(typekey.CoverageType.TC_COMP, CoverageSubtype.TC_COMP_VD, claim)
      }
    }
  ...
  override function getExposure(claim:Claim, glLossInfo:LiabilityLossInfo_Type, incident:Incident) : Exposure {
    var exposure:Exposure
    var coverageType = getCoverageType(glLossInfo.Coverages)
    if(coverageType!=null) {
      exposure = newExposure(coverageType, ExposureType.TC_GENERALDAMAGE, claim)
    } else {
      exposure = newExposure(typekey.CoverageType.TC_GL, ExposureType.TC_GENERALDAMAGE, claim)
    }
  ...
  override function getExposure(claim:Claim, propLossInfo:PropertyLossInfo_Type, incident:PropertyIncident) : Exposure {
    var exposure:Exposure
    ...
    var coverageType = getCoverageType(propLossInfo.Coverages)
    if(coverageType!=null) {
      exposure = newExposure(coverageType, exposureType, claim)
    } else {
      exposure = newExposure(typekey.CoverageType.TC_BLDG, exposureType, claim)
    }
    exposure.Incident =  incident
    exposure.LossParty = LossPartyType.TC_THIRD_PARTY
    return exposure
  ...
  override function getExposure(claim:Claim, wcLossInfo:WorkCompLossInfo_Type, incident:Incident) : Exposure {
    var exposure:Exposure
    ...
    if(wctype.equalsIgnoreCase("BecameMed") or wctype.equalsIgnoreCase("Medical")) {
      exposure = newExposure(CoverageType.TC_WM, CoverageSubtype.TC_WM_WCID, claim)
    } 
    else if(wctype.equalsIgnoreCase("Indemnity") or wctype.equalsIgnoreCase("LostTime")) {
      exposure = newExposure(CoverageType.TC_WI, CoverageSubtype.TC_WI_LW, claim)
    }
    else {
      exposure = newExposure(CoverageType.TC_WL, CoverageSubtype.TC_WL_EL,claim)
    }
    exposure.Incident = incident
    return exposure
  }
  ...
  override function getExposure(claim:Claim, injuryInfo:ClaimsInjuredInfo_Type, incident:InjuryIncident) : Exposure {
    var exposure = newExposure(claim) 
    exposure.ExposureType = ExposureType.TC_BODILYINJURYDAMAGE
    if(claim.Policy.PolicyType==PolicyType.TC_AUTO_PER or claim.Policy.PolicyType==PolicyType.TC_AUTO_COMM)
      exposure.PrimaryCoverage = typekey.CoverageType.TC_ABI
      ...
      exposure.PrimaryCoverage = typekey.CoverageType.TC_GL
    exposure.LossParty = LossPartyType.TC_THIRD_PARTY
    exposure.Incident = incident
    return exposure
  }
  ...
  private function containsPropertyOwner(autoLossInfo:AutoLossInfo_Type) : boolean {
    for(claimsPartyRef in autoLossInfo.ClaimsPartyRefs) {
      if((claimsPartyRef as ClaimsParty_Type).hasRole(AcordUtil.ROLE_PROPERTY_OWNER))
        return true;
    }
    return false;
  }
  ...
  private function getCoverageType(coverages:List<Coverage_Type>) : CoverageType {
  ...
  private function matchCoverage(policy:Policy, coverageType: CoverageType) : Coverage {
     var coverages = policy.getCoveragesByCoverageType(coverageType)
  ...
  private function setOtherCoverageInfo(exposure:Exposure, claimPartyInfo:ClaimsPartyInfo_Type) {
    if(claimPartyInfo.OtherOrPriorPolicys!=null and claimPartyInfo.OtherOrPriorPolicys.size() > 0) {
      exposure.OtherCoverage = true
      var sb = new StringBuilder()
      for(otherPolicy in claimPartyInfo.OtherOrPriorPolicys) {
