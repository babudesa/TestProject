...
class FatalityClaimIndicatorMethodsImpl extends ClaimIndicatorMethodsImpl {
  ...
  construct(inIndicator : FatalityClaimIndicator) {
    super(inIndicator, "indicator_icon_fatality.gif")
  }
  ...
  override property get Text() : String {
    return Indicator.IsOn ? displaykey.Web.Claim.FatalityClaimIndicator.Fatalities.OnText 
            : displaykey.Web.Claim.FatalityClaimIndicator.Fatalities.OffText
  }
  ...
  override property get HoverText() : String {
    ...
    if (WorkersCompensationClaim) {
      ...
      var injuredParties = claim.InjuryIncidentsOnly
          ...
          .map(\ i -> i.injured)
      result = injuredParties.sort().join(displaykey.Web.Claim.FatalityClaimIndicator.FatalitySeperator)
    }
    return result
  }
  
  private property get WorkersCompensationClaim() : boolean {
    return Indicator.Claim.LossType == "WC"
  }
