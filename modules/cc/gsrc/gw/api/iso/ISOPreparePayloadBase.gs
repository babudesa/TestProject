...
class ISOPreparePayloadBase {
   ...
   * ids. Finally returns the modified message payload and the SuppressMetchInd
   * flag.
   */
  function preparePayload(message : Message, properties : ISOProperties) : PayloadState {
    ...
    var isoAcord = xsd.iso.ACORD.parse(message.Payload)
    ...
    return new PayloadState(isoAcord.asUTFString(), suppressMatches)
  }
