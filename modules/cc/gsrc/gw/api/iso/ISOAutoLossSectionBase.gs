package gw.api.iso
uses xsd.iso.AutoLossInfo
uses xsd.iso.Registration
...
class ISOAutoLossSectionBase extends ISOLossSection {  
  ...
  protected override function createLossSection(claimantParty : ClaimsParty) : IXMLNodeWithID {
    ...
    SearchRequest.AddRequest.AutoLossInfos.add(lossInfo)
    return lossInfo    
  }
  ...
  protected function createAutoLossInfo(claimantParty : ClaimsParty, vehicleIncident : VehicleIncident) : AutoLossInfo {
    ...
    var autoLossInfo = new xsd.iso.AutoLossInfo()
    ...
    if (registration != null) {
      autoLossInfo.VehInfo.Registrations.add(registration)
    }
