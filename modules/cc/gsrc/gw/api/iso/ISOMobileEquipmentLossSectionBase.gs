...
class ISOMobileEquipmentLossSectionBase extends ISOLossSection {  
  ...
  override function verifyCanPopulate(existingLossSections : List<ISOLossSectionBase>) : String {
    return SearchRequest.AddRequest.PropertyLossInfo != null
      ? displaykey.Java.Error.ISO.NoMultiplePropertyLossInfo : null
  }
  ...
  protected function addPropertyLossInfo(claimantParty : ClaimsParty) : PropertyLossInfo {
    ...
    lossInfo.ItemInfos.add(createItemInfo())
    return lossInfo
  } 
  ...
  protected function createItemInfo() : ItemInfo{
    ...
    if (vehicle == null) {
      throw new ISOBadRequestException(displaykey.Java.Error.ISO.MissingProperty("vehicle", "MobileEquipment", Exposure.Claim.ClaimNumber ))
    }
