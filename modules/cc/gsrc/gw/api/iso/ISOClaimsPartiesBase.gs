package gw.api.iso
uses xsd.iso.ClaimsParty
uses xsd.iso.Communications_PhoneInfo
uses xsd.iso.NameInfo
uses xsd.iso.Communications
uses xsd.iso.GeneralPartyInfo
uses xsd.iso.DriversLicense
uses xsd.iso.TaxIdentity
uses xsd.iso.StringCd
uses java.util.HashMap
uses java.lang.IllegalArgumentException
uses xsd.iso.ClaimsPartyRelationship2
...
class ISOClaimsPartiesBase {
  ...
  public function setDriver(driver : Person) {
    ...
    if (claimsParty != null && driver.LicenseNumber.HasContent && claimsParty.ClaimsDriverInfo == null) {
      claimsParty.ClaimsDriverInfo.DriversLicenses = {createDriversLicense(driver)}
    }
  }
   ...
   */
  protected function isAddressReadyForISO(address : AddressBase, role : ISOClaimsPartyRole) : boolean {
  ...
  protected function addNewClaimsParty(owner : ClaimsParty,
    ...
    var claimsParty = new xsd.iso.ClaimsParty()
    ...
    _isoRequest.AddRequest.ClaimsPartys.add(claimsParty)
    return claimsParty
  }
  ...
  protected function setRole(owner : ClaimsParty, claimsParty : ClaimsParty, role : ISOClaimsPartyRole) {
    if (role != getRole(claimsParty)) {
      claimsParty.ClaimsPartyInfo.ClaimsPartyRoleCds = {createClaimsPartyRoleCd(role)}
  ...
  protected function removeRelationships(claimsParty : ClaimsParty) {
    _isoRequest.AddRequest.ClaimsPartyRelationships.removeWhere(\ r -> r.ClaimsParty2Ref == claimsParty)
  }
   ...
   * role
   */
  protected function createClaimsPartyRoleCd(role : ISOClaimsPartyRole) : StringCd {
    var roleCd = new xsd.iso.StringCd()
    if (!role.IsAcordStandard) {
      roleCd.codelistref = _isoRequest.findOrCreateCodeList(ISOCodeList.CLAIMS_PARTY_ROLE_CODE.Id)
    }
    roleCd.Value = role.Code
    return roleCd
  }
  ...
  protected function createGeneralPartyInfo(contact : Contact, full : boolean) : GeneralPartyInfo {
    var generalPartyInfo = new xsd.iso.GeneralPartyInfo()  
  ...
  protected function createCommunications(contact :  Contact, full : boolean) : Communications {
    var communications = new xsd.iso.Communications()
  ...
  protected function createPhoneInfo(phoneType : String, communicationType : String, number : String) : 
    ...
    var phoneInfo = new xsd.iso.Communications_PhoneInfo()
    phoneInfo.PhoneTypeCd = phoneType
    var isoCommunicationType = new xsd.iso.StringCd()
    isoCommunicationType.Value = communicationType
    phoneInfo.CommunicationUseCds.add(isoCommunicationType)
  ...
  protected function createNameInfo(contact : Contact, full : boolean) : NameInfo {
    var nameInfo = new xsd.iso.NameInfo()
    if (contact typeis Person) {
      nameInfo.Choice.PersonName.Surname = _isoRequest.truncateString(contact.LastName, 30) // 30
      nameInfo.Choice.PersonName.GivenName = _isoRequest.truncateString(contact.FirstName, 20) // 20
      var middleName = new xsd.iso.String()
      ...
      nameInfo.Choice.CommlName.CommercialName = _isoRequest.truncateString(company.Name, 55)
    }
  ...
  protected function createTaxIdentity(contact : Contact) : TaxIdentity {
    ...
    var taxIdentity = new xsd.iso.TaxIdentity()
  ...
  protected function getRole(claimsParty : ClaimsParty) : ISOClaimsPartyRole {
    return ISOClaimsPartyRole.byCode(claimsParty.ClaimsPartyInfo.ClaimsPartyRoleCds[0].Value)
  }
}
