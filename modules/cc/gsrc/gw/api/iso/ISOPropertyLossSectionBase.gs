...
uses xsd.iso.PropertyLossInfo_ClaimsSubjectInsuranceInfo
uses java.math.BigDecimal
uses gw.xml.xsd.IXMLNodeWithID
uses xsd.iso.ClaimsPayment
uses xsd.iso.PropertyLossInfo
uses xsd.iso.AdjusterPartyInfo
uses xsd.iso.PropertySchedule
...
class ISOPropertyLossSectionBase extends ISOLossSection {  
  ...
  protected override function createAndAddClaimsPayment(claimantParty : ClaimsParty) : ClaimsPayment {
    ...
    if (SearchRequest.AddRequest.PropertyLossInfo.ClaimsSubjectInsuranceInfos.Count > 0) {
      ...
      claimsPayment.ClaimsPaymentCovInfos[0].CoverageCd = "PRPD" 
   ...
   * party.
   */
  protected function addPropertyLossInfo(claimantParty : ClaimsParty) : xsd.iso.PropertyLossInfo {
    var lossInfo = SearchRequest.AddRequest.PropertyLossInfo
    if (lossInfo == null) {
    ...
    if (Exposure.LostPropertyType != null) {
      lossInfo.PropertySchedules.add(createPropertySchedule())
    }
    return lossInfo
  }
