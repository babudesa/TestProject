 ...
 */
@ReadOnly
abstract class CCAddressOwnerBase implements CCAddressOwner {
  ...
  construct(prop : IEntityPropertyInfo) {
    ...
    if (not entity.Address.Type.isAssignableFrom(_property.FeatureType)) {
  ...
  override property get Address() : Address {
    ...
    if (result == null and LocationUtil.isCurrentLocationEditable()) {
  ...
  override property get SelectedCountry() : Country {
    return this.CoreAddress.Country != null ? this.CoreAddress.Country : DefaultCountry
  }
   ...
   * is usually of the form displaykey.Web.Address.XX.Name, where XX is the
   * country code.
   */
  ...
  property get AddressFields() : CountryAddressFields {
    return CountryAddressFields.forCountry(InputSetMode)
  }
  ...
  protected function addOriginalValue(addressList : Address[]) : Address[] {
    ...
    if (originalId != null) {
      original = Owner.Bundle.loadByKey(originalId) as Address
    }
