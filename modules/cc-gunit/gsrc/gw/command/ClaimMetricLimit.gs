...
class ClaimMetricLimit extends BaseCommand  {
  ...
  function auto_per() {
    gw.transaction.Transaction.runWithNewBundle(\ limitBundle -> {
      createClaimMetricLimits(limitBundle, PolicyType.TC_AUTO_PER)
    })
  }

  function wc() {
    gw.transaction.Transaction.runWithNewBundle(\ limitBundle -> {
      createClaimMetricLimits(limitBundle, PolicyType.TC_COMP)
    })
  }
  ...
  private function createLimits(limitBundle : Bundle, policyType : PolicyType, metricType : typekey.ClaimMetric, metricUnit : MetricUnit, metricCategory : ClaimMetricCategory) {
    ...
    if(metricUnit == typekey.MetricUnit.TC_NUMERIC || metricUnit == typekey.MetricUnit.TC_DAYS) {
      ...
      var currencyRedValue = CurrencyAmount.getStrict(new BigDecimal("300.00"), Currency.TC_USD)
      for (claimTier in claimTiers) {
        ...
        currencyYellowValue = currencyYellowValue.addStrict(new BigDecimal("500.00"))
        currencyRedValue = currencyRedValue.addStrict(new BigDecimal("1000.00"))
      }
