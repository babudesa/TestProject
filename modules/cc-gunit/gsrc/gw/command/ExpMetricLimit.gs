...
class ExpMetricLimit extends BaseCommand  {
  ...
  function auto_comm() {
    gw.transaction.Transaction.runWithNewBundle(\ limitBundle -> {
      createExposureMetricLimits(limitBundle, PolicyType.TC_AUTO_COMM)
    })
  }
  
  function auto_per() {
    gw.transaction.Transaction.runWithNewBundle(\ limitBundle -> {
      createExposureMetricLimits(limitBundle, PolicyType.TC_AUTO_PER)
    })
  }
  
  function prop_comm() {
    gw.transaction.Transaction.runWithNewBundle(\ limitBundle -> {
      createExposureMetricLimits(limitBundle, PolicyType.TC_PROP_COMM)
    })
  }
  ...
  function gen_liability() {
    gw.transaction.Transaction.runWithNewBundle(\ limitBundle -> {
      createExposureMetricLimits(limitBundle, PolicyType.TC_GEN_LIABILITY)
    })
  }

  function wc() {
    gw.transaction.Transaction.runWithNewBundle(\ limitBundle -> {
      createExposureMetricLimits(limitBundle, PolicyType.TC_COMP)
    })
  }
  ...
  private function createLimits(limitBundle : Bundle, policyType : PolicyType, metricType : typekey.ExposureMetric, metricUnit : MetricUnit) {
    ...
    if(metricUnit == typekey.MetricUnit.TC_NUMERIC || metricUnit == typekey.MetricUnit.TC_DAYS) {
      ...
      var currencyTargetValue = CurrencyAmount.getStrict(new BigDecimal("100.00"), Currency.TC_USD)
      var currencyYellowValue = CurrencyAmount.getStrict(new BigDecimal("200.00"), Currency.TC_USD)
      var currencyRedValue = CurrencyAmount.getStrict(new BigDecimal("400.00"), Currency.TC_USD)
      for (exposureTier in exposureTiers) {
        new MoneyExposureMetricLimitBuilder()
              ...
              .create(limitBundle)
        currencyTargetValue = currencyTargetValue.addStrict(new BigDecimal("100.00"))
        currencyYellowValue = currencyYellowValue.addStrict(new BigDecimal("100.00"))
        currencyRedValue = currencyRedValue.addStrict(new BigDecimal("100.00"))
      }
