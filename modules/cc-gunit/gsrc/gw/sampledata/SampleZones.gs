...
class SampleZones  extends SampleDataBase {
  ...
  static function loadZone(code:String, type:ZoneType, country:Country) : Zone {
    return find(zone in Zone where zone.Code == code and zone.ZoneType == type and zone.Country == country).single()
  }
  ...
  function buildZones(bundle : Bundle) {
    ...
    var county1 = new ZoneBuilder().withName("San Mateo").withCode("CA:San Mateo").withZoneType("county").withCountry("us").create()
    ...
    // cities
    var city1 = new ZoneBuilder().withName("San Mateo").withCode("CA:San Mateo").withZoneType("city").withCountry("us").create()
