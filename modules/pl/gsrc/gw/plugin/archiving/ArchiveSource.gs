...
class ArchiveSource extends BaseLocalDocumentContentSource implements IArchiveSource  {
  ...
  public static final var ARCHIVE_SUBDIR_WITH_SEPARATOR : String = ARCHIVE_SUBDIR + File.separator
  ...
  var _status : StatusImpl = new StatusImpl()
   override property get Status() : ArchiveSourceInfo {
     return _status
   }
  var _initialStoreStatus : ArchiveSourceStatus
  var _initialRetrieveStatus : ArchiveSourceStatus
  
  override function setParameters(parameters : Map) {
    if (!ServerModeUtil.isDev() && !ServerModeUtil.isTest()) {
      Logger.ARCHIVING.error("This plugin was not meant for production")
    }
    _initialStoreStatus = getStatusFromParameters(parameters, STORE_INITIAL_STATE_PARAM)
    _initialRetrieveStatus = getStatusFromParameters(parameters, RETRIEVE_INITIAL_STATE_PARAM)
    super.setParameters(parameters)
    refresh()
  }
  
  private function getStatusFromParameters(parameters : Map, param : String) : ArchiveSourceStatus {
    var str = parameters.get(param) as String
    if (str == null) {
      return "available"
    }
    var rtn = ArchiveSourceStatus.get(str)
    if (rtn == null) {
   ...
   override function refresh() {
    _status.StoreStatus = _initialStoreStatus
    _status.RetrieveStatus = _initialRetrieveStatus
     _status.DeleteStatus = "available"
  ...
  override function store(graph : InputStream, info : RootInfo) {
    if (_status.StoreStatus != "available") {
...
//    super.addDocument(new ByteArrayInputStream(os1.toByteArray()), wrapped) 
      ...
      Logger.ARCHIVING.debug("Saved ${info.RootPublicID} graph to ${wrapped.getSubDirForDocument()}")
    }
    catch (e : Throwable) {
      throw e
    }
  ...
  override function storeFinally(info : RootInfo, finalStatus : ArchiveFinalStatus, cause : List<String>) {
    Logger.ARCHIVING.debug("Store of " + info.RootPublicID + ", finalStatus=" + finalStatus);
...
//  }
  ...
  override function retrieve(info : RootInfo) : InputStream {
    if (_status.RetrieveStatus != "available") {
  ...
  override function retrieveFinally(info : RootInfo, finalStatus : ArchiveFinalStatus, cause : List<String>) {
    Logger.ARCHIVING.debug("On " + info.RootPublicID + " retrieve: final=" + finalStatus);
...
//  }
    ...
    if (prefix == "send") {
      if (parts.Count > 2) {
        if (parts[0] == "send") {
          ...
          case "avail" : 
            Logger.ARCHIVING.info("qaPublicIdAsCommand:on command " + publicId)
           _status.StoreStatus = "available"
           break; 
          case "queue" : 
            _status.StoreStatus = "queue"; 
            Logger.ARCHIVING.info("qaPublicIdAsCommand:on command " + publicId)
            break; 
          }
        }
          ...
          switch (parts[1]) {
            case "off" : 
              Logger.ARCHIVING.info("qaPublicIdAsCommand:on command " + publicId + " turning off retreiveals")
              _status.RetrieveStatus = "manually"; 
              break; 
            case "on" : 
              Logger.ARCHIVING.info("qaPublicIdAsCommand:on command " + publicId + " turning on retreiveals")
              _status.RetrieveStatus = "available"; 
              break; 
          }
        }
        else if (parts[0] == "del") {
          switch (parts[1]) {
            case "off" : 
              Logger.ARCHIVING.info("qaPublicIdAsCommand:on command " + publicId + " turning off deletes")
              _status.DeleteStatus = "manually"; 
              break; 
            case "on" : 
              Logger.ARCHIVING.info("qaPublicIdAsCommand:on command " + publicId + " turning on deletes")
              _status.DeleteStatus = "available"; 
              break; 
          }
        }
      ...
      if (parts.Count > 2 && parts[0] == "recv") {
        switch (parts[1]) {
          case "fail" : 
            _status.RetrieveStatus = "failure"
  ...
  override function updateInfoOnDelete(info : RootInfo) : List<Pair<IEntityType, List<Key>>> {
   ...
   override function delete(info : RootInfo) {   
     if (_status.DeleteStatus != "available") {
