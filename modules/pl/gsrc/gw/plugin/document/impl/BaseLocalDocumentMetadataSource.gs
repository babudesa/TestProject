...
@Export
abstract class BaseLocalDocumentMetadataSource
    implements IDocumentMetadataSourceBase {
      
  private var _rootDir : File as readonly RootDir
  private var _tempDir : File as readonly TempDir
  ...
  */
  ...
  protected property get MetadataDir() : File {
    return DocumentConfigUtil.getMetadataDir().JavaFile
  }
  
  protected property get MetadataFiles() : File[] {
    return MetadataDir.listFiles().where(\ file -> file.File)
  }
  ...
  */  
  private function retrieveDocument( metadataFile : File, doThrow : boolean) : Document {
    ...
    try {
      ...
      var result = DocumentsUtilBase.deserializeXmlTo(xml, Document) as Document
      Logger.DOCUMENT.debug( \ ->  { return "BaseLocalDocumentMetadataSource retrieveDocument id={result.ID} docUID={result.DocUID} publicId={result.PublicId}"} )
      return result
    } 
    catch (e : Exception) {
      if (doThrow) {
        ...
        Logger.DOCUMENT.warn("Could not get document metadata from " + metadataFile, e)
        return null
      }
    }
    finally {
  ...
  */
  private function getFilePathForUniqueId(uniqueId : String, metadata : boolean ) : String {
  ...
  */
  protected function storeDocument(document : Document) {
    var metadataFile = getMetadataFileForUniqueId(document.PublicID)
    Logger.DOCUMENT.debug( \ ->  { return "BaseLocalDocumentMetadataSource.store id={document.Id} docUUD={document.DocUID} file={metadataFile}"} )
    var out : Writer
    try {
      var xmlContent = DocumentsUtilBase.serializeDocumentToXMLString(document)
      if (not metadataFile.exists()) {
        metadataFile.createNewFile()
      }
      out = StreamUtil.getOutputStreamWriter( new BufferedOutputStream(new FileOutputStream(metadataFile)) )
