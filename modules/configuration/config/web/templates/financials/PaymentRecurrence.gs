...
Recurrence.prototype.setRecurring = function(isRecurring) {
  ...
  if (this.isRecurring) { 
    DHTML.getElementById('result_tr').style.display = 'table-row';
  } else {
    DHTML.getElementById('result_tr').style.display = 'none';
  }
...
<table cellpadding="3" cellspacing="0" width="124" class="dv">
  <tr>
  <td class="inputLabel"><%=displaykey.NVV.Financials.NewPaymentInstructions.Check.SendDate%></td><td>
    <td>
  ...
  <tr id="result_tr" style="display: none;">
    ...
    <td>
      <table cellpadding="3" cellspacing="0" id="recurrencePattern">
        ...
        <tr>
          <td class="inputLabel"><%=displaykey.Velocity.NewPayment.Recurrence.Pattern%>
          <td>
