	...
	<xsl:template name="Insert_PropLost">
		...
		<xsl:if test="string-length($Display_Value) > 0">
			...
			<xsl:call-template name="Insert_Data">
				<xsl:with-param name="Data_Name"></xsl:with-param>
		...
		<xsl:if test="string-length(string(normalize-space($incept))) &gt; 0">
			...
			<tr>
				...
				<td nowrap="nowrap" >
					...
					<b>
					<font size="2" face="Arial">
					<xsl:value-of select="$Title1"/>:</font></b></td><td>
...
-->
	<xsl:template name="Insert_Simple_Data_Label">
		<xsl:param name="Data_Name"/>
			<font size="2"  face="Arial">
			<b><xsl:value-of select="$Data_Name"/>:</b>	
			</font>	
	</xsl:template>	
<!--
*********************************************************************************
				...
				&#160;&#160;
					<xsl:if test="string-length(normalize-space($Data_Value)) &gt; 0">
						<font size="2"  face="Arial">
								...
								<b><xsl:value-of select="$Data_Value"/></b>
						</font>
					</xsl:if>
				</td>						
		...
		<xsl:if test="string-length(normalize-space($Data_Value)) &gt; 0">
			<tr>	
				...
				<td align="left" colspan="2">
					<li>
					<font size="2" face="Arial">
					<xsl:value-of select="$Data_Value"/></font>
					</li>
				</td>
...
-->
	<xsl:template name="Insert_Data_And_Check_For_More_Matches">
		...
		<xsl:if test="string-length(normalize-space($Data_Value)) &gt; 0">
			...
			<!-- check to see if the id has a value then check to see if this id has additional matches -->
			<xsl:if test="string-length(normalize-space($Data_Value_ID)) &gt; 0">
				<xsl:if test="count($ClaimRoot/MatchInfo/AdditionalMatchInfo[@idref = $Data_Value_ID]) &gt; 0">
					<xsl:call-template name="Insert_Data">
						<xsl:with-param name="Data_Name">							
						</xsl:with-param>
						<xsl:with-param name="Data_Value">
							*** More matches found on this <xsl:value-of select="$Data_Name"/><xsl:text> outside this report ***</xsl:text>
		...
		<xsl:if test="string-length(string(normalize-space($Addr1))) &gt; 0">
			<tr>
				...
				&#160;&#160;
					<b>
					<font size="2" face="Arial">
					<xsl:value-of select="$Title"/>:</font></b></td>
				<td align="left">
					<font size="2" face="Arial">
...
-->
	<xsl:template name="Insert_Anchor_Link">
		...
		<a>
		     <xsl:attribute name="href" >#<xsl:value-of select="$AnchorName"/>
      		</xsl:attribute>
...
-->
	<xsl:template name="Insert_More_Matches">
		...
		<xsl:if test="$ClaimInvestAR_Node = $Match_Id">
		<!--<xsl:if test="$ClaimInvestAR_Node/MatchInfo/AdditionalMatchInfo/@idref = $Match_Id">-->
			<tr><td></td>
			<td>
				<xsl:text>*** More matches on this </xsl:text>
				<xsl:value-of select="$Match_Desc"/><xsl:text> outside this report ***</xsl:text>
			</td></tr>
		</xsl:if>
	</xsl:template>
