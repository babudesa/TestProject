...
class BaseDocumentTemplateSource implements InitializablePlugin, IDocumentTemplateSource {
  ...
  static property get StemSearchPropNames() : Set<String> { return PROPNAMES_TEST_WITH_STARTSWITH }
  ...
  override function getDocumentTemplates( date: Date, valuesToMatch : Map, maxResults : int ) : IDocumentTemplateDescriptor[]  {
    ...
    Logger.DOCUMENT.debug(\ -> "AA")
    var descriptors = getTemplateDescriptors(locale)
    Logger.DOCUMENT.debug(\ -> "BB")
    ...
    for (var descriptor in descriptors) {
      if (descriptorIsInEffect(descriptor, date) and descriptorMatchesAllValues(descriptor, map)) {
        Logger.DOCUMENT.debug(\ -> "CC")
        if (descriptorIsValid(descriptor)) {
          Logger.DOCUMENT.debug(\ -> "DD")
  ...
  public static function extractLocale(valuesToMatch : Map) : ILocale {
  ...
  */
  public static function descriptorIsInEffect(descriptor : IDocumentTemplateDescriptor, date : Date) : boolean {
    if (date != null) {
  ...
  public static function descriptorMatchesAllValues(descriptor : IDocumentTemplateDescriptor, valuesToMatch : Map) : boolean {
    for (var propName in valuesToMatch.keySet() as Set<String>) {
      if (propName.equalsIgnoreCase( AVAILABLE_SYMBOLS_PROPNAME ) ) { // available vs required symbol check, 
        ...
        // for Worker's Comp templates
        if(propName=="lob" and valuesToMatch.get(propName).toString()==LossType.TC_AGRIWC.Code){
          for(lob in LossType.TF_WC_EXPORT_LOSSTYPES.TypeKeys){
            // if lob metadata of template contains at least 1 wc lob from TF_WC_EXPORT_LOSSTYPES filter - display this template
            if(descriptor.getMetadataPropertyValue(propName).toString().containsIgnoreCase(lob.Code)){
              if(valuesToMatch.containsKey("keywords") and valuesToMatch.get("keywords")!=null){
                if(not match("keywords", descriptor.getMetadataPropertyValue("keywords"), valuesToMatch))
                  return false
              }
              return true
            }
   ...
   private static function match(propName : String, value : Object, valuesToMatch : Map) : boolean {
     try {
   ...
   private static function matchStartsWith(propName : String, value : Object, valuesToMatch : Map) : boolean {
     try {
  ...
  */
  protected function getTemplateDescriptors(locale : ILocale) : List<IDocumentTemplateDescriptor> {
    // deal with the cache
  ...
  private function getLocalTemplateDescriptors(locale : ILocale) : List<IDocumentTemplateDescriptor> {
    ...
    files.each(\ s -> Logger.DOCUMENT.debug(\ -> s));
