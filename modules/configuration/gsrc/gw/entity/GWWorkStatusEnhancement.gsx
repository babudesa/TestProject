...
enhancement GWWorkStatusEnhancement : entity.WorkStatus {
   ...
   * 
   * 6.12.15 - cmullin - WC Config / Requirements Change - changed the error message to 
   * display the dates of the existing, conflicting WorkStatus rather than the dates of the 
   * conflicting WorkStatus that the user is adding.
   */
  function validateNonConflicting(data:EmploymentData) {
    for (ws in data.WorkStatusChanges) {
      if (this != ws and this.overlaps(ws)) {
        throw new DisplayableException(displaykey.Web.Claim.NewWorkStatusPopup.Conflict(ws.StatusDate.format("short"), 
          ws.StatusEndDate!=null ? ws.StatusEndDate.format("short") : displaykey.Term.Date.Present))
      }
    }
  }
}
