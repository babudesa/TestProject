...
uses xsd.iso.req.ClaimsPartyInfo_ClosedDt
uses xsd.iso.req.ClaimsPartyInfo_SuitFiledInd
...
abstract class ISOLossSectionBase {
  ...
  public function populate() : xsd.iso.req.ACORD {
    ...
    if(Exposure.State == ExposureState.TC_CLOSED && Exposure.CloseDate != null){
      var closedDt = new ClaimsPartyInfo_ClosedDt()
      closedDt.Value = SearchRequest.Translate.formatClaimDate(Exposure.CloseDate)
      closedDt.idref = lossSection.NodeID
      claimantParty.ClaimsPartyInfo.ClosedDts.add(closedDt)
    }  
    ...
    if (ShouldAddInvestigationInfo) {
      ...
      if (investigationInfo != null) {
        ...
        for(var recoveryInfo in investigationInfo.RecoveryInfos) {
          recoveryInfo.ItemRef = lossSection
        }
      }
    }
    
    if(Exposure.ex_InSuit){
      var suitFiledInd = new ClaimsPartyInfo_SuitFiledInd()
      suitFiledInd.Value = "1"
      suitFiledInd.idref = lossSection.NodeID
      claimantParty.ClaimsPartyInfo.SuitFiledInds.add(suitFiledInd)
    }
   ...
   */
  abstract protected function createLossSection(claimantParty : ClaimsParty) : IXMLNodeWithID
  ...
  protected property get IsFirstParty() : boolean {
    return Exposure.LossParty == "insured"
  }
   ...
   * Defect 5671: Do not send this aggregate anymore.
   */
  ...
  protected function createAndAddClaimsPayment(claimantParty : ClaimsParty) : ClaimsPayment {
    ...
    SearchRequest.AddRequest.ClaimsPayments.add(claimsPayment)
  ...
  protected function createClaimsPayment(claimantParty : ClaimsParty) : xsd.iso.req.ClaimsPayment {
    ...
    claimsPayment.ClaimsPaymentCovInfos.add(coverageInfo)
    
    var payment = Exposure.PaymentAmount
    var hasPayment = payment != null and payment.compareTo(BigDecimal.ZERO) != 0
  ...
  */
  ...
  protected function createRecoveryInfo() : RecoveryInfo {
    ...
    if (Exposure.VehicleIncident.RecovDate != null
      ...
      recoveryInfo.RecoveryStatusCds = {createRecoveryStatusCode(recoveryConditionCode)}
      return recoveryInfo
    }
    return null
  }
  ...
  protected function createRecoveryStatusCode(recoveryConditionCode : String) : StringCd {
    var roleCd = new xsd.iso.req.StringCd()
...
//      }
  ...
  protected function findClaimsParty(role : ISOClaimsPartyRole) : ClaimsParty {
     return SearchRequest.AddRequest.ClaimsPartys.firstWhere(
             \ c -> c.ClaimsPartyInfo.ClaimsPartyRoleCds.hasMatch(
                     \ s -> s.Value == role.Code))                           
  }
