...
class ISOClaimSearchRequestBase extends ISORequest {
  ...
  protected function createClaimsOccurrence(insurerId : String, incidentDescription : String) {   
    ...
    // Defect 9126 - kniese - Add SiteID to messages
    AddRequest.ClaimsOccurrence.com_iso_SiteId.Text = "000000011"
   // Changes made for defect 8700(added 99-103)
   if(incidentDescription!=null && incidentDescription.length()<=50){
    AddRequest.ClaimsOccurrence.IncidentDesc = incidentDescription} 
    else if(incidentDescription!=null && incidentDescription.length()>50){
    AddRequest.ClaimsOccurrence.com_iso_NewExtLossDesc =truncateString(incidentDescription, 200)
    }  
    ...
    if (witnessContactRole != null
      ...
      AddRequest.ClaimsOccurrence.com_iso_AccidentWitnessedInd = ISOConstants.YES
    } else {
      AddRequest.ClaimsOccurrence.com_iso_AccidentWitnessedInd = ISOConstants.NO
    }
  }
  ...
  protected function createProbableIncurredAmt(amount : BigDecimal) {
    if (amount != null and amount != BigDecimal.ZERO) {
      ...
      var piaa : xsd.iso.req.ClaimsOccurrence_ProbableIncurredAmt
      if (AddRequest.ClaimsOccurrence.ProbableIncurredAmts.HasElements) {
        piaa = AddRequest.ClaimsOccurrence.ProbableIncurredAmts[0]
      } else {
        piaa = new xsd.iso.req.ClaimsOccurrence_ProbableIncurredAmt()
        AddRequest.ClaimsOccurrence.ProbableIncurredAmts.add(piaa)
      }
  ...
  public function constructClaimLevelLossSection(exposure : Exposure) : ISOLossSection  {
    ...
    var constructor = lossSectionType.TypeInfo.getConstructor({ISOClaimSearchRequest, entity.Exposure})
    if (constructor == null) {
      throw new IllegalStateException(displaykey.Java.Error.ISO.NoConstructorForClaim(lossSectionType))
    }
  ...
  protected function verifyLossSection(lossSection : ISOLossSection) : String {
    ...
    if (!verifyPolicyTypeMatches(lossSection)) {
      error = displaykey.Java.Error.ISO.MismatchPolicyTypeCode
    } else {
  ...
  protected function logErrorMessageForClaimLevelSearch() {
    if (_lossSectionErrors.Count > 0 ) {
      ...
      ISOProperties.LOGGER.info(loggingMessage)
    }
  }
