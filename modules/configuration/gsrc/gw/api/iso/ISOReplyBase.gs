...
uses xsd.iso.resp.MatchDetails
...
class ISOReplyBase {
  ...
  var _reply : xsd.iso.resp.ClaimInvestigationAddRs_Sequence as readonly Reply
  ...
  construct(xml : String) {
    ...
    try {
      _reply = collapseSequences(xsd.iso.resp.ClaimInvestigationAddRs.parse(xml, false))
    } catch (e : Throwable ) {
 ...
 /*
  * Take an xsd.iso.resp.ClaimInvestigationAddRs and move all property values to a single sequence for easier use.
  * Also moves SIUParties onto the MatchDetails that precedes them. This helps associate the SIUParties and MatchDetails
  */
  private function collapseSequences(addRs : xsd.iso.resp.ClaimInvestigationAddRs) : xsd.iso.resp.ClaimInvestigationAddRs_Sequence {
    var finalSequence = new xsd.iso.resp.ClaimInvestigationAddRs_Sequence()
    for(seq in addRs.Sequences) {
      if(seq.MatchDetailss != null and seq.MatchDetailss.size() > 0) {
        var finalMatchDetails : xsd.iso.resp.MatchDetails
        //Move the MatchDetails manually if there are any. Keep track of the last for SIUParties
        for(matchDetails in seq.MatchDetailss) {
          finalSequence.MatchDetailss.add(matchDetails)
          finalMatchDetails = matchDetails
        }

        //Clear the sequence MatchDetails so they aren't re-added
        seq.MatchDetailss.clear()

          if(finalMatchDetails != null) {
            //Now iterate of SIUParties and add them to the MatchDetails
            for(siuParty in seq.com_iso_SIUPartys) {
              finalMatchDetails.com_iso_SIUPartys.add(siuParty)
          }
        }
      }

      //For the rest of the properties we will place them on our new sequence so they can be easily accessed
      //Note that this will also include the com_iso_SIUParties, we put them in both locations
      for(prop in xsd.iso.resp.ClaimInvestigationAddRs_Sequence.Type.TypeInfo.Properties) {
        var value = prop.Accessor.getValue(seq)
        if(value != null) {
          if(value typeis Collection) {
            (prop.Accessor.getValue(finalSequence) as Collection).addAll(value)
          } else {
            prop.Accessor.setValue(finalSequence, value)
          }
        }
      }
    }
   ...
   */  
  protected function getMatchReasons(matchDetail : xsd.iso.resp.MatchDetails) : String {
   ...
   */
  protected function populateMatchReportFromXML(matchReport : ISOMatchReport, matchDetail : xsd.iso.resp.MatchDetails) {
    ...
    if (miscParties.HasElements) {
      ...
      if (phoneInfos.HasElements and phoneInfos.first().PhoneNumbers.HasElements) {
        matchReport.InsurerPhone = phoneInfos.first().PhoneNumbers.first().Value
      }
    }
  }
}
