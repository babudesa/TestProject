...
class PLDClaimsRptgCoreProc  {
  var env = Environment.getInstance()
  
  construct() {
  }
 
  public function PLDClaimsDataProcess(runType : String, clmMap : HashMap, lastRunTran : Date, sumFileNameFull : String, tranFileNameFull : String, balFileNameFull : String, runDate : String): void{
     ...
     if (env == Environment.PROD or env == Environment.UAT or env == Environment.LOCAL) {
       excelSumFileName = "PLDClaimsSummary" + String.valueOf(year) + monthStr + dayStr + ".txt"
       excelTranFileName = "PLDClaimsTrans"  + String.valueOf(year) + monthStr + dayStr + ".txt"
       excelBalFileName = "PLDClaimsBalancing"  + String.valueOf(year) + monthStr + dayStr + ".txt"
     }
     else {
       excelSumFileName = "PLDClaimsSummary" + env + String.valueOf(year) + monthStr + dayStr + ".txt"
       excelTranFileName = "PLDClaimsTrans"  + env + String.valueOf(year) + monthStr + dayStr + ".txt"
       excelBalFileName = "PLDClaimsBalancing"  + env + String.valueOf(year) + monthStr + dayStr + ".txt"
     }
    // var summaryUniqueIDFileName = "PLDSummaryUniqueID.txt"
    // var summaryUniqueIDFileNamePath = ""
     System.out.print(excelSumFileName)
     if (env == Environment.LOCAL) {
       var excelSumFileNameIncPath = "c:/work/PLDClaimsSummary"
       var excelTranFileNameIncPath = "c:/work/PLDClaimsTrans"
       var excelBalFileNameIncPath = "c:/work/PLDClaimsBalancing"
     }
     else
     if (env <> Environment.DEV3) {
       var excelSumFileNameIncPath = "/app/tomcat/tomcat-cccore/apache-tomcat-6.0.30/temp/PLDClaimsSummary"
       var excelTranFileNameIncPath = "/app/tomcat/tomcat-cccore/apache-tomcat-6.0.30/temp/PLDClaimsTrans"
       var excelBalFileNameIncPath = "/app/tomcat/tomcat-cccore/apache-tomcat-6.0.30/temp/PLDClaimsBalancing"
   //    summaryUniqueIDFileNamePath = "/app/tomcat/tomcat-cccore/apache-tomcat-6.0.30/temp/"
     }
     else
     {
       var excelSumFileNameIncPath = "/app/tomcat/tomcat-ccdev3/apache-tomcat-6.0.30/temp/PLDClaimsSummary"
       var excelTranFileNameIncPath = "/app/tomcat/tomcat-ccdev3/apache-tomcat-6.0.30/temp/PLDClaimsTrans"
       var excelBalFileNameIncPath = "/app/tomcat/tomcat-ccdev3/apache-tomcat-6.0.30/temp/PLDClaimsBalancing"
   //    summaryUniqueIDFileNamePath = "/app/tomcat/tomcat-ccdev3/apache-tomcat-6.0.30/temp/"
     ...
     try{
       ...
       var PLDRptQuery = gw.api.database.Query.make(Claim)
       var cctlLobCodeTable = PLDRptQuery.compare("LOBCode", Equals, LOBCode.TC_PROFLIABDIV);

    // gw.transaction.Transaction.runWithNewBundle(\ bundle -> PLDClaimsDataExtract())
    // var transactionTable = PLDRptQuery.outerJoin(Transaction, "Claim")
    // var transactionLineitemTable = transactionTable.outerJoin(TransactionLineItem, "Transaction")
       var resultSet = PLDRptQuery.select().orderBy(\ c -> c.ClaimNumber)
       ...
       pwSum.println("CLM_NO^NDTD_LOSS_RESERVE^DTDW_LOSS_RESERVE^DTDW_EXPENSE_RESERVE^DTDO_LOSS_RESERVE^DTDO_EXPENSE_RESERVE^NDTD_EXPENSE_RESERVE^CVG_EXPENSE_RESERVE^CLM_CREATED_DATE^" + 
           ...
           "PRODUCT^POL_NO^POL_EFFECTIVE_DATE^CLAIM_STATUS^CLOSED_DATE^REOPEN_DATE^REOPEN_REASON^FACT_SUMMARY^AREA_OF_PRACTICE^PROJECT_TYPE^INDUSTRY_TYPE^UNDERLYING_DAMAGES^" +
           "CONTRACT_TYPE^EO_LOSS_RESERVE^EO_EXPENSE_RESERVE^EO_LOSS_PAID^EO_EXPENSE_PAID^JURIS_STATE^JURIS_CODE|")
       ...
       pwTran.println("CLM_NO^CLM_TRANS_NO^CLM_PROCESS_DATE^CLM_TRANS^NDTD_LOSS_RESERVE_ADJ^DTDW_LOSS_RESERVE_ADJ^DTDW_EXPENSE_RESERVE_ADJ^DTDO_LOSS_RESERVE_ADJ^DTDO_EXPENSE_RESERVE_ADJ^" + 
           ...
           "PAYEE_NAME^INVOICE_AMOUNT^UNIQUE_ID^PRODUCT^POL_NO^POL_EFFECTIVE_DATE^EO_LOSS_RESERVE_ADJ^EO_EXPENSE_RESERVE_ADJ^EO_LOSS_PAID_ADJ^EO_EXPENSE_PAID_ADJ|")  
        ...
        if ((aClaim.ClaimNumber.substring(0, 1) != "T" and runType <> "Claim") or (runType == "Claim" and clmMap.containsKey(aClaim.ClaimNumber))) {
          ...
          if (aClaim.ValidationLevel == ValidationLevel.TC_PAYMENT) {
            if (env == Environment.UAT) {
              if  (aClaim.CreateUser.Department == "PLD" and aClaim.CreateTime > cutOffDate) {
                validClmStatus = true
              }
              else {
          ...
          for (exposure in exposureList) {
             ...
             if (exposure.ExposureType == ExposureType.TC_PL_EO) {
                amount = availReservesCalculator.getAmount( exposure, CostType.TC_CLAIMCOST )
                reserveLossEO = reserveLossEO + amount
                amount = totalPaymentsCalculator.getAmount( exposure, CostType.TC_CLAIMCOST )
                paidLossEO = paidLossEO + amount
                //recoveriesLoss = totalRecoveriesCalculator.getAmount(exposure, CostType.TC_CLAIMCOST) as Double
                //reserveLossEO = reserveLossEO + recoveriesLoss
                //paidLossEO = paidLossEO - recoveriesLoss
                amount = availReservesCalculator.getAmount( exposure, CostType.TC_EXPENSE )
                reserveExpEO = reserveExpEO + amount
                amount = totalPaymentsCalculator.getAmount( exposure, CostType.TC_EXPENSE )
                paidExpEO = paidExpEO + amount
                //recoveriesExp = totalRecoveriesCalculator.getAmount(exposure, CostType.TC_EXPENSE) as Double
                //reserveExpEO = reserveExpEO + recoveriesExp
                //paidExpEO = paidExpEO - recoveriesExp
             }
                 ...
                 if (!trans.LineItems.IsEmpty) {
                   ...
                   for (li in trans.LineItems) {
                     ...
                     if (li.Transaction.CostType == CostType.TC_EXPENSE or (li.Transaction.Exposure.ExposureType == ExposureType.TC_EL_DUTYDEFOTSDLIMITS or li.Transaction.Exposure.ExposureType == ExposureType.TC_EL_DUTYDEFWTHNLIMITS or
                         li.Transaction.Exposure.ExposureType == ExposureType.TC_EL_INDEMNITY or li.Transaction.Exposure.ExposureType == ExposureType.TC_PL_EO)) {
                       ...
                       if ((li.Transaction typeis Payment or li.Transaction typeis Recovery)  and (li.Transaction.Status == TransactionStatus.TC_AWAITINGSUBMISSION or li.Transaction.Status == TransactionStatus.TC_ISSUED or li.Transaction.Status == TransactionStatus.TC_PENDINGAPPROVAL
                          ...
                          if (exposure.ExposureType == ExposureType.TC_PL_EO and li.Transaction.CostType == CostType.TC_CLAIMCOST) {
                             reserveLossEO = reserveLossEO + tranAmt
                             paidLossEO = paidLossEO - tranAmt
                          }
                          else
                          if (exposure.ExposureType == ExposureType.TC_PL_EO and li.Transaction.CostType == CostType.TC_EXPENSE) {
                             reserveExpEO = reserveExpEO + tranAmt
                             paidExpEO = paidExpEO - tranAmt
                          }
                       }
                     }
                    ...
                    if (li.Transaction.Exposure.ExposureType == ExposureType.TC_PL_EO and rptTran == true) {
                       tranStr = tranStr.concat("^0.00")
                       tranStr = tranStr.concat("^0.00")
                       tranStr = tranStr.concat("^0.00")
                       tranStr = tranStr.concat("^0.00")
                    ...
                    if (rptTran == true) {
                      ...
                      tranStr = tranStr.concat("^" + aClaim.Policy.PolicyNumber)
                      tranStr = tranStr.concat("^" + aClaim.Policy.EffectiveDate)
                      if (li.Transaction.Exposure.ExposureType == ExposureType.TC_PL_EO) {

                        if (li.Transaction typeis Reserve and li.Transaction.CostType == CostType.TC_CLAIMCOST  ) {
                           tranStr = tranStr.concat("^" + df.format(li.Transaction.Amount))
                          tranStr = tranStr.concat("^0.00")
                          tranStr = tranStr.concat("^0.00")
                          tranStr = tranStr.concat("^0.00" + "|")
                        ...
                        if (li.Transaction typeis Reserve and li.Transaction.CostType == CostType.TC_EXPENSE  ) {
                          tranStr = tranStr.concat("^0.00")
                          tranStr = tranStr.concat("^" + df.format(li.Transaction.Amount))
                          tranStr = tranStr.concat("^0.00")
                          tranStr = tranStr.concat("^0.00" + "|")
                        ...
                        if (li.Transaction typeis Payment and li.Transaction.CostType == CostType.TC_CLAIMCOST) {
                          if (paymentType <> "Supplemental") {
                            tranStr = tranStr.concat("^" + df.format(-1 * li.Transaction.Amount))
                          }
                          else
                        ...
                        if (li.Transaction typeis Payment and li.Transaction.CostType == CostType.TC_EXPENSE) {
                          tranStr = tranStr.concat("^0.00")
                          if (paymentType <> "Supplemental") {
                            tranStr = tranStr.concat("^" + df.format(-1 * li.Transaction.Amount))
                          }
          ...
          if (rptClaim == true) {
           ...
           // }
            ...
            if (aClaim.ex_DetailLossCause <> null) { 
          ...
          //pw.print(",") //Claim Retentions (Deductible)
            ...
            pwSum.print("^" + generateNewTranID())
            pwSum.print("^" + aClaim.Policy.PolicyType)
            pwSum.print("^" + aClaim.Policy.PolicyNumber)
            ...
            pwSum.print("^" + aClaim.AreaofPracticeExt.DisplayName)
            pwSum.print("^" + aClaim.ProjectCategoryExt.DisplayName)
            pwSum.print("^" + aClaim.IndustryCategoryExt.DisplayName)
            pwSum.print("^" + aClaim.UnderlyingDamagesExt.DisplayName)
            pwSum.print("^" + aClaim.ContractTypeExt.DisplayName)
            pwSum.print("^" + df.format(reserveLossEO))
            pwSum.print("^" + df.format(reserveExpEO))
            pwSum.print("^" + df.format(paidLossEO))
            pwSum.println("^" + df.format(paidExpEO))
            if (legalExpenseFound) {
              ...
              if (saveMatter.StateExt <> null) {
             	pwSum.println("^" + saveMatter.StateExt)
             	if (saveMatter.CourtType <> null) {
              		pwSum.println("^" + saveMatter.CourtType + "|")
                }
                else {
                    pwSum.println("^|")
                }
              ...
              if (jurisdictionState <> null) {
              	pwSum.println("^" + jurisdictionState + "^|")
              }
              else {
               pwSum.println("^^|")
              }      
       ...
       util.ErrorHandling.GAICErrorHandling.logError(null, "PLDClaimsRptImpl.gs", ex, null)
    ...
    try {
       ...
       //pwSum.println(sumRecCnt + "^" + sumTotRsvA + "^" + sumTotRsvB + "^" + sumTotRsvB2 + "^" + sumTotRsvC + "^" + sumTotpaidLoss + "^" + sumTotPaidB + "^" + sumTotpaidLoss + "^" + sumTotpaidLoss + "^" + sumTotPaidC + "^" + sumTotCedLossRes + "^" + sumTotCedLAERes + "^" + sumTotCedLossPd + "^" + sumTotCedLAEPd)
		...
		//var mailProps = Environment.getInstance().createPropertiesForPath(PROPERTY_PATH, mailPropsFile);
                ...
                //mailProps.load(new FileInputStream("modules\\configuration\\config\\PLDclaimsreporting\\mail.properties"))
		...
		print(env.toString())
                if (env <> Environment.PROD) {
		  msg.setSubject("PLD Claims Reporting File " + env);
                }
                else
                {
                  msg.setSubject("PLD Claims Reporting File ")
                }
     		var multipart = new MimeMultipart()
     		
          	messageBodyPart.setText("Attached you'll find a file containing all PLD Claims Reporting data produced since the last report")
...
//	}
  ...
  public function generateNewTranID(  ) : String {
    return buildTranNumber("PLDTranNumberSequence")
  }
