...
class ELDClaimsReporting extends BatchProcessBase {
  ...
  construct() {
    super(BatchProcessType.TC_ELDCLAIMSRPTGPROCESS)
    gw.api.util.Logger.logInfo("ELDClaimsReporting.gs called")
  }
  
 Override function doWork(): void {
   Transaction.runWithNewBundle(\ bundle -> ELDClaimsDataExtract(), "su")
 }
 
  public function ELDClaimsDataExtract(): void{
     ...
     processHistoryQuery.compare("ProcessType", Equals, BatchProcessType.TC_ELDCLAIMSRPTGPROCESS )
     ...
     var lobPrefix = "ELD"
     var lobCode = LOBCode.TC_EXECLIABDIV
