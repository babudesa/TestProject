...
class ConversionActivityGeneratorProcess extends BatchProcessBase {
  ...
  construct() {
    super(BatchProcessType.TC_CONVERSIONACTIVITYGENERATOR)
  }
  ...
  * draft, and have a LoadCommandID in order to be eligible for activity generation.
  ...
  */
  private property get ClaimsToCheck(): List<Key>{
    ...
    qryClaim.compare("LoadCommandID", NotEquals, null)
 ...
 /**
  * Runs each eligible claim through the ConversionActivityGeneratorBatchRules 
  */ 
  override function doWork(){
    OperationsExpected = this.ClaimsToCheck.Count
    gw.api.util.Logger.logInfo("ConversionActivityGeneratorProcess.doWork() - Job has " + OperationsExpected + " records to process...")
    ...
    while(_iter.hasNext()){
      ...
      try{
          if(!this.TerminateRequested){        
              ...
              gw.api.util.Logger.logError("ConversionActivityGeneratorProcess - Terminate Requested...")
              break
          }            
               
      }catch(e){
        incrementOperationsFailed()
        gw.api.util.Logger.logError("ConversionActivityGeneratorProcess - Failed to generate activities for claim: " + claim.ClaimNumber)
  ...
  private function generateActivities(claim : Claim){
    if(claim.isNativeORUpdatedExt()){
      //run the claim through the activity rule set
      rules.ActivityGeneratorBatch.ConversionActivityGeneratorBatchRules.invoke(claim)
  ...
  private function printSummary(){
    gw.api.util.Logger.logInfo("Conversion Activity Generator Batch Process is Complete.")
