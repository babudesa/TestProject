...
class ELDClaimsRptgCoreProc  {
  var env = Environment.getInstance()
  
  construct() {
  }
 
  public function ELDClaimsDataProcess(runType : String, clmMap : HashMap, lastRunTran : Date, sumFileNameFull : String, tranFileNameFull : String, balFileNameFull : String, runDate : String): void{
     ...
     if (env == Environment.PROD or env == Environment.UAT or env == Environment.LOCAL) {
       excelSumFileName = "ELDClaimsSummary" + String.valueOf(year) + monthStr + dayStr + ".txt"
       excelTranFileName = "ELDClaimsTrans"  + String.valueOf(year) + monthStr + dayStr + ".txt"
       excelBalFileName = "ELDClaimsBalancing"  + String.valueOf(year) + monthStr + dayStr + ".txt"
     }
     else {
       excelSumFileName = "ELDClaimsSummary" + env + String.valueOf(year) + monthStr + dayStr + ".txt"
       excelTranFileName = "ELDClaimsTrans"  + env + String.valueOf(year) + monthStr + dayStr + ".txt"
       excelBalFileName = "ELDClaimsBalancing"  + env + String.valueOf(year) + monthStr + dayStr + ".txt"
     }
    ...
    // var summaryUniqueIDFileNamePath = ""
     ...
     if (env == Environment.LOCAL) {
       ...
       var excelBalFileNameIncPath = "c:/work/ELDClaimsBalancing"
     }
     else
     if (env <> Environment.DEV3) {
       var excelSumFileNameIncPath = "/app/tomcat/tomcat-cccore/apache-tomcat-6.0.30/temp/ELDClaimsSummary"
       var excelTranFileNameIncPath = "/app/tomcat/tomcat-cccore/apache-tomcat-6.0.30/temp/ELDClaimsTrans"
       var excelBalFileNameIncPath = "/app/tomcat/tomcat-cccore/apache-tomcat-6.0.30/temp/ELDClaimsBalancing"
   //    summaryUniqueIDFileNamePath = "/app/tomcat/tomcat-cccore/apache-tomcat-6.0.30/temp/"
     }
     else
     {
       var excelSumFileNameIncPath = "/app/tomcat/tomcat-ccdev3/apache-tomcat-6.0.30/temp/ELDClaimsSummary"
       var excelTranFileNameIncPath = "/app/tomcat/tomcat-ccdev3/apache-tomcat-6.0.30/temp/ELDClaimsTrans"
       var excelBalFileNameIncPath = "/app/tomcat/tomcat-ccdev3/apache-tomcat-6.0.30/temp/ELDClaimsBalancing"
   //    summaryUniqueIDFileNamePath = "/app/tomcat/tomcat-ccdev3/apache-tomcat-6.0.30/temp/"
     ...
     try{
       ...
       var ELDRptQuery = gw.api.database.Query.make(Claim)
       var cctlLobCodeTable = ELDRptQuery.compare("LOBCode", Equals, LOBCode.TC_EXECLIABDIV);
    ...
    // var transactionLineitemTable = transactionTable.outerJoin(TransactionLineItem, "Transaction")
       var resultSet = ELDRptQuery.select().orderBy(\ c -> c.ClaimNumber)
       ...
       pwSum.println("CLM_NO^NDTD_LOSS_RESERVE^DTDW_LOSS_RESERVE^DTDW_EXPENSE_RESERVE^DTDO_LOSS_RESERVE^DTDO_EXPENSE_RESERVE^NDTD_EXPENSE_RESERVE^CVG_EXPENSE_RESERVE^CLM_CREATED_DATE^" + 
           ...
           "PRODUCT^POL_NO^POL_EFFECTIVE_DATE^CLAIM_STATUS^CLOSED_DATE^REOPEN_DATE^REOPEN_REASON^FACT_SUMMARY^JURIS_STATE^JURIS_CODE|")
       ...
       for (aClaim in resultSet) {
        ...
        if ((aClaim.ClaimNumber.substring(0, 1) != "T" and runType <> "Claim") or (runType == "Claim" and clmMap.containsKey(aClaim.ClaimNumber))) {
          ...
          //pw.print(",") //Claim Retentions (Deductible)
            ...
            pwSum.println("^" + factSummary) //Case Summary
            if (legalExpenseFound) {
       ...
       util.ErrorHandling.GAICErrorHandling.logError(null, "ELDClaimsRptImpl.gs", ex, null)
    ...
    try {
       ...
       //pwSum.println(sumRecCnt + "^" + sumTotRsvA + "^" + sumTotRsvB + "^" + sumTotRsvB2 + "^" + sumTotRsvC + "^" + sumTotpaidLoss + "^" + sumTotPaidB + "^" + sumTotpaidLoss + "^" + sumTotpaidLoss + "^" + sumTotPaidC + "^" + sumTotCedLossRes + "^" + sumTotCedLAERes + "^" + sumTotCedLossPd + "^" + sumTotCedLAEPd)
		...
		print(env.toString())
                if (env <> Environment.PROD) {
		  msg.setSubject("ELD Claims Reporting File " + env);
                }
                else
                {
                  msg.setSubject("ELD Claims Reporting File ")
                }
     		var multipart = new MimeMultipart()
     		
          	messageBodyPart.setText("Attached you'll find a file containing all ELD Claims Reporting data produced since the last report")
...
//	}
  ...
  public function generateNewTranID(  ) : String {
    return buildTranNumber("ELDTranNumberSequence")
  }
