...
class MAClaimsReporting extends BatchProcessBase {
  ...
  construct() {
    super(BatchProcessType.TC_MACLAIMSRPTGPROCESS)
    gw.api.util.Logger.logInfo("MAClaimsReporting.gs called")
  }
  
 Override function doWork(): void {
   Transaction.runWithNewBundle(\ bundle -> MAClaimsDataExtract(), "su")
 }
 
  public function MAClaimsDataExtract(): void{
     ...
     processHistoryQuery.compare("ProcessType", Equals, BatchProcessType.TC_MACLAIMSRPTGPROCESS )
     ...
     var lobPrefix = "MA"
     var lobCode = LOBCode.TC_MERGACQU
