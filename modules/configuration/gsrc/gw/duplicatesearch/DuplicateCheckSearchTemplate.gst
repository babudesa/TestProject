...
var encryptionPlugin = gw.plugin.Plugins.get(IEncryption)
var needsOr = false
...
if (check.FirstPayment.Exposure.Claimant != null){
%>
AND claimantcontact.ID = <%= helper.makeParam("Exposure.ClaimantDenormID", check.FirstPayment.Exposure.Claimant.ID) %>
<% } %>
<%
// Added code below to only check transactions that are not negative so that voided and stopped checks get detected as duplicates
%>
AND cc_transactionlineitem_2.TransactionAmount >= 0
AND cc_transactionlineitem_2.TransactionAmount = <%= helper.makeParam("TransactionLineItem.TransactionAmount", check.getGrossAmount()) %>

GROUP BY
  cc_check_head.ID

<%
function calculateCheckIdsToIgnore() : Key[] {
  var result : Key[] = null
