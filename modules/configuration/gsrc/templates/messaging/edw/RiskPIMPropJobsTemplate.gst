...
<%@ params(theclaim : Claim, thepolicy : Policy, therisk : LocationBasedRU, theexposure : Exposure, objStatus : String, riskCat : String, cvrg : String, riskType : String, eventName : String, actualjobsite : String, thejobsitenumber : String) %>
<Risk>
  <PublicID><%=therisk.PublicID%></PublicID>
  <ObjectStatus><%=objStatus%></ObjectStatus>
  <UnVerifiedRisk>
    <% if ((therisk as JobsiteRUExt).JobsiteDescExt != null) { %> 
    <LocationNumber><%=(therisk as JobsiteRUExt).JobsiteDescExt%></LocationNumber>
    <% } else { %>
  ...
  <%}%>
  <% if (thejobsitenumber == null) {%>
      <RelToRisk><%=actualjobsite%>.JOBSITE</RelToRisk>
  <%} else {%>
      <RelToRisk><%=thepolicy.PublicID%>.<%=thejobsitenumber%>.JOBSITE</RelToRisk>
  <%}%>
</Risk>