...
<%@ params(theclaim : Claim, therisk : LocationBasedRU, objStatus : String, riskCat : String, cvrg : String, riskType : String, eventName : String, thelocationnumber : String, thecoverages : List<Coverage>) %>
<Risk>
  <% if (therisk.Property.RiskTypeExt != null and therisk.Property.RiskTypeExt == "FRMLOC") {%>
  <PublicID><%=therisk.PublicID%>.<%=therisk.Property.RiskTypeExt%></PublicID>
  <%} else {%>
  <PublicID><%=theclaim.Policy.PublicID%>.<%=thelocationnumber%>.FRMLOC</PublicID>
  <%}%>
