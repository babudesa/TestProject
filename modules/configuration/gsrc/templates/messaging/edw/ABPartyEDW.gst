...
<Transaction>
  ...
  <FinanciallyProcessedIndicator>false</FinanciallyProcessedIndicator>
    ...
    <Phones>
      ...
      <% if (thecontact.CellPhoneExt != null) {%>
      <Phone>
        <%
        ext = thecontact.CellPhoneExt.indexOf( "x") < 0 ? 0 : thecontact.CellPhoneExt.indexOf( "x");
        cntr = ext == 0 ? 0 : 1;
        ext = ext == 0 ? thecontact.CellPhoneExt.length() : ext;
        %>
        <PhoneNumber><%=thecontact.CellPhoneExt.substring( 0, ext - cntr ).replaceAll("-", "")%></PhoneNumber>
        ...
        <%}%>   
        <% if (cntr != 0) {%>    
        <ExtensionNumber><%=thecontact.CellPhoneExt.substring( ext + 1, thecontact.CellPhoneExt.length() )%></ExtensionNumber>
        <%}%>
      </Phone>
      <%}%>
    </Phones>
