...
<tns:RegionOffice>00001</tns:RegionOffice>
  ...
  <tns:Name><%=theContact.DisplayName%></tns:Name>
   <% if (theContact.DateOfBirth != null) {%> 
  <tns:DateOfBirth><%=(new java.text.SimpleDateFormat("MMddyyyy")).format(theContact.DateOfBirth)%></tns:DateOfBirth>
	<% } else { %>
  ...
  <tns:SocialSecurityNumber><%=util.gaic.OFAC.OFACFunctions(theContact.TaxID)%></tns:SocialSecurityNumber>
	<% } else { %>
