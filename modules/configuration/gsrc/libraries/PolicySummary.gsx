...
enhancement PolicySummary : entity.PolicySummary
{
  ...
  /*
    Function sets the ex_Agency of the given PolicySummary by calling the PRM integration.
    Author: Zach Thomas
    Date: 03/23/2010
    Updated: 2/10/15 erawe/dougH
  */
  function setProducerFromPRM(producerCode : String){
    var needToLoad = false;
    try{
      needToLoad = (this.ex_Agency == null)
    }catch(e){
      needToLoad = true;
    }
    if(needToLoad){
      var prmProducer : PRMProducer = new PRMProducer();
      try{
        var agency = prmProducer.findProducer(producerCode);
         this.ex_Agency = agency;
      }
      catch(e) {
